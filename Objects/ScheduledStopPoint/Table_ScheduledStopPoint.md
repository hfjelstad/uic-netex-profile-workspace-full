...
|  privateCodes | Container | 0..1 | 0..1 | Preferred NeTEx v2.0 container for typed external identifiers | ScheduledStopPoint/privateCodes  |
|  PrivateCode (@type) | String | 1..n | 1..n | External code for inter-system matching. In UIC migration: `type="uicCode"` carrying the station's UIC code. Enables deterministic matching to corresponding typed codes on StopPlace without changing `@id`. | ScheduledStopPoint/privateCodes/PrivateCode  |
|  PrivateCode | String | 0..1 | 0..1 | Legacy single-code form kept for compatibility; prefer `privateCodes/PrivateCode` in v2.0 datasets | ScheduledStopPoint/PrivateCode  |
